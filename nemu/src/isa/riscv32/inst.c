...
#define BTRACE(inst,taken) MUXDEF(CONFIG_BTRACE, btrace(inst, taken), 0)

void btrace(uint32_t inst, bool taken){
  printf("BTRACE: inst: 0x%08x taken: %d\n", inst, taken);
}
...
#define INSTPAT_MATCH(s, name, t, ...) { \
  ...
  //printf("nemu: cpu.grp[2]=%08x\n", cpu.gpr[2]);
   ...
   INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, BTRACE(INSTPAT_INST(s), src1 != src2), s->dnpc = (src1 != src2 ? s->pc + imm : s->dnpc));
   INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = SEXT(Mr(src1 + imm, 4), 32));
   INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = src1 + src2);
   INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << SHAMT_LONG);
   INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, BTRACE(INSTPAT_INST(s),src1 == src2), s->dnpc = (src1 == src2 ? s->pc + imm : s->dnpc));
   INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, BTRACE(INSTPAT_INST(s),(word_t)src1 < (word_t)src2), s->dnpc = ((word_t)src1 < (word_t)src2 ? s->pc + imm : s->dnpc));
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(rd) = SEXT((uint32_t)src1 >> SHAMT, 32));
   ...
   INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, BTRACE(INSTPAT_INST(s),(sword_t)src1 < (sword_t)src2), s->dnpc = ((sword_t)src1 < (sword_t)src2 ? s->pc + imm : s->dnpc));
   INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, BTRACE(INSTPAT_INST(s),((word_t)src1 >= (word_t)src2)), s->dnpc = (((word_t)src1 >= (word_t)src2 ? s->pc + imm : s->dnpc)));
   
   INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, BTRACE(INSTPAT_INST(s),((word_t)src1 >= (word_t)src2)), s->dnpc = ((word_t)src1 >= (word_t)src2 ? s->pc + imm : s->dnpc));
