...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // //R-type 寄存器-寄存器操作指令
   ...
   INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = src1 >> SEXT(BITS(src2, 4, 0),5));
...
#ifdef CONFIG_RV64
   INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , R, R(rd) = src1 >> SEXT(SHAMT_LONG, SHAMT_LONG_LEN));
   INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , R, R(rd) = src1 << SHAMT_LONG);
#else 
   INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , R, R(rd) = src1 >> SEXT(SHAMT_LONG, SHAMT_LONG_LEN));
   INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , R, R(rd) = src1 << SHAMT_LONG);
