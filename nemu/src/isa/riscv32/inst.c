...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = BITS(src1 + imm,31,0)); //addi(li,mv)
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xoir   , I, R(rd) = src1 ^ imm); //XORI rd, rs1, -1 =  NOT rd, rs
 ...
 // INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2) s->dnpc = s->pc + imm);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = ((int32_t)BITS(src1, 31, 0) >= (int32_t)BITS(src2, 31, 0) ? s->pc + imm : s->dnpc));

  //R-type 寄存器-寄存器操作指令
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , R, R(rd) = SEXT(BITS(src1 + src2, 31, 0),64));
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(rd) = BITS(src1,31,0) - BITS(src2,31,0));
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = BITS(src1,31,0) + BITS(src2,31,0));
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(rd) = BITS(src1,31,0) & BITS(src2,31,0));
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = BITS(src1,31,0) | BITS(src2,31,0));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = SEXT(BITS(src1, 31, 0) >> (int32_t)BITS(src2, 4, 0),32));
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , R, R(rd) = src1 >> SEXT(SHAMT,32));
  ...
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , R, R(rd) = src1 >> (uint32_t)SHAMT_LONG);
