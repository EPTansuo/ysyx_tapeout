...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I ,R(rd) = ((uint32_t)BITS(src1,31,0) < (uint32_t)BITS(imm,31,0) ? 1 : 0));
  INSTPAT("??????? ????? ????? 010 ????? 01100 11", slti   , I ,R(rd) = ((int32_t)BITS(src1,31,0) < (int32_t)BITS(imm,31,0)? 1 : 0));
