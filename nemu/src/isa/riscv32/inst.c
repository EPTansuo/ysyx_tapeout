...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01100 11", slti   , I ,R(rd) = ((sword_t)src1 < (sword_t)imm) ? 1 : 0);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & (~1);\
   /*  printf("s->pc:0x%016lx, s->dnpc:0x%016lx, src1: 0x%016lx, imm %ld\n",s->pc,s->dnpc, src1, imm)*/); //最低位置零
  INSTPAT("??????? ????? ????? 011 ????? 00000 11", ld     , I, R(rd) = Mr(src1 + SEXT(imm,64), 8));
 ...
 // INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2) s->dnpc = s->pc + imm);
  ...
  //INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if((word_t)src1 < (word_t)src2) s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = ((word_t)src1 < (word_t)src2 ? s->pc + imm : s->dnpc));
