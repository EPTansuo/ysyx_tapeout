...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + imm, 1), XLEN));
  // INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2), XLEN));
  // INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2));
   INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = SEXT(Mr(src1 + imm, 4), XLEN); \
    printf("SEXT(Mr(src1 + imm, 4), XLEN) = 0x%lx",SEXT(Mr(src1 + imm, 4), XLEN)));
  ...
  // INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(rd) = SEXT((uint32_t)src1 << BITS(src2, 4, 0), 64));
  // INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = ((word_t)src1 < (word_t)src2 ? 1 : 0));
  // INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , R, R(rd) = SEXT(BITS(src1, 31, 0) >> (uint32_t)SHAMT, 64));
  // INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = SEXT(BITS(src1, 31, 0) >> (int32_t)BITS(src2, 4, 0), 64));
  // INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , R, R(rd) = src1 >> SHAMT_LONG);
  // INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , R, R(rd) = src1 << SHAMT_LONG);
  // INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = (sword_t)src1 * (sword_t)src2);
  // INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, R(rd) = SEXT(BITS(src1 * src2, 31, 0), 64));
  ...
  // INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(rd) = SEXT(BITS((sword_t)src1 / (sword_t)src2, 31, 0), 64));
