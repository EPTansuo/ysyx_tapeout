...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // //R-type 寄存器-寄存器操作指令
   ...
   INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , R, R(rd) = (sword_t)src1 >> (sword_t)SHAMT_LONG);
   INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , R, R(rd) = src1 << SHAMT_LONG);
#ifndef CONFIG_RV32
   //INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, int64_t tmp = (int64_t)(int32_t)src1 * (int64_t)src2; R(rd) = BITS(tmp, 63, 32));
